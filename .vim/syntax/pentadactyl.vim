...
syn keyword pentadactylCommand loadplugins lpl gr[oup] ! run Clistk[eys] Clk
    ...
    \ delmac[ros] delm[arks] delqm[arks] dels[tyle] dia[log] dlc[lear]
    \ doautoa[ll] do[autocmd] downl[oads] dl ec[ho] echoe[rr] echom[sg] el[se]
    \ elsei[f] elif em[enu] en[dif] fi exe[cute] exit x exta[dd] extde[lete]
    ...
    \ nm[ap] nno[remap] noh[lfind] no[remap] norm[al] nunm[ap] o[pen] pa[geinfo]
    ...
    \ setg[lobal] setl[ocal] sideb[ar] sb[ar] sbop[en] sil[ent] sitesearch
    \ so[urce] st[op] stopa[ll] sty[le] styled[isable] styd[isable]
...
syn keyword pentadactylOption activate act altwildmode awim autocomplete au
    ...
    \ extendedhinttags eht fileencoding fenc findcase fc followhints fh
    \ guioptions go helpfile hf hintinputs hin hintkeys hk hintmatching hm
    \ hinttags ht hinttimeout hto history hi iskeyword isk jumptags jt
    \ loadplugins lpl mapleader ml maxitems messages msgs newtab nextpattern
    \ pageinfo pa passkeys pk passunknown pu popups pps previouspattern
    \ runtimepath rtp sanitizeitems si sanitizeshutdown ss sanitizetimespan sts
    \ scroll scr shell sh shellcmdflag shcf showmode smd showstatuslinks ssli
    \ showtabline stal strictfocus sf suggestengines timeoutlen tmol titlestring
